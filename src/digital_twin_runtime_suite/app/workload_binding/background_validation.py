...
class BackgroundAirflowValidationCoordinator:
    ...
    async def run(
        ...
        self._report_diagnostic(
            owner="AIRFLOW BACKGROUND VALIDATION",
            process="DATASET PREFLIGHT",
            state="START",
            current_workload=self._current_binding.workload_mode,
            order=", ".join(_dataset_identity(dataset) for dataset in self._datasets),
            mode="sequential",
        )
        try:
            while self._queue:
                ...
                if result.status == "CANCELLED":
                    ...
                    if result.reused:
                        ...
                        self._report_diagnostic(
                            ...
                            state="PASS",
                            selector=job.selector,
                            attempt=f"{job.attempt}/{self.MAX_ATTEMPTS}",
                            receipt=result.receipt.signature.compact_digest,
                        )
                    ...
                    self._report_diagnostic(
                        ...
                        selector=job.selector,
                        attempts=f"{job.attempt}/{self.MAX_ATTEMPTS}",
        ...
        self._report_diagnostic(
            ...
            background_validated=len(validated),
            priority_validated=len(self._priority_validated),
            total_validated=len(validated | self._priority_validated),
            reused=len(reused),
            failed=len(terminal_failures),
            flow_attached=self._flow_attached(),
        )
        return BackgroundValidationResult(
            validated=len(validated | self._priority_validated),
            failed=len(terminal_failures),
    ...
    async def _run_job(self, job: _ValidationJob) -> _JobResult:
        ...
        try:
            ...
            if cached_receipt:
                ...
                owner = (
                    "AIRFLOW BACKGROUND VALIDATION"
                    if job.priority == "BACKGROUND"
                    else "AIRFLOW VALIDATION"
                )
                self._report_diagnostic(
                    owner=owner,
                    process="DATASET PREFLIGHT",
                    ...
                    selector=job.selector,
                    receipt_source=lookup.receipt_source,
                    current_cache_location=lookup.cache_location,
                    validation_executed=False,
                    receipt=cached_receipt.signature.compact_digest,
                )
            else:
                self._report_diagnostic(
                    owner="AIRFLOW BACKGROUND VALIDATION",
    ...
    def _report_diagnostic(
        ...
        **details: object,
        ...
        """Send semantic preflight facts through the shared line formatter."""
