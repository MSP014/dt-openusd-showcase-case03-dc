...
def diff_flow_snapshots(
    reference: dict[str, Any], dtrs: dict[str, Any]
) -> dict[str, Any]:
    ...
    def collect_differences(reference_value, dtrs_value, path=""):
        if isinstance(reference_value, dict) and isinstance(dtrs_value, dict):
            differences = []
            for key in sorted(set(reference_value) | set(dtrs_value)):
                if key == "prim_path":
                    continue
            ...
            return differences
        if reference_value != dtrs_value:
            return [{"path": path, "reference": reference_value, "dtrs": dtrs_value}]
        return []

    expected = []
    suspicious = []
    reference_roles = reference.get("roles", {})
    dtrs_roles = dtrs.get("roles", {})
    for role in sorted(set(reference_roles) | set(dtrs_roles)):
