...
def test_cache_load_verifies_speed_primvar_with_usd_time_codes(
    ...
    _install_validated_cache_contract(monkeypatch, paths, runtime.metadata)
...
def _runtime(tmp_path: Path, stage: "_Stage") -> tuple["_LoadRuntime", object]:
    ...
    metadata = SimpleNamespace(states=states, sample_count=2, settings=object())
...
def _install_validated_cache_contract(monkeypatch, paths, metadata) -> None:
