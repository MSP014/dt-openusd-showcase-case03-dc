...
def test_visualization_mode_request_and_result_emit_readable_log_blocks():
    workflow = _workflow()

    async def request() -> None:
        assert workflow.request_mode_from_ui(VisualizationMode.STREAMLINES)
        await workflow._visualization_task

    asyncio.run(request())

    assert "process=MODE TRANSITION | state=REQUEST" in workflow.logs[0]
    assert "requested_mode=Streamlines" in workflow.logs[0]
    assert "process=MODE TRANSITION | state=COMPLETE" in workflow.logs[1]
    assert "committed_mode=Streamlines" in workflow.logs[1]


def test_streamlines_then_normal_preserves_workflow_sequence():
    controller = _Controller()
    workflow = _workflow(controller)

    asyncio.run(workflow._request_mode(VisualizationMode.STREAMLINES))
    asyncio.run(workflow._request_mode(VisualizationMode.NORMAL))

    assert controller.requested_modes == [
        VisualizationMode.STREAMLINES,
        VisualizationMode.NORMAL,
    ]
    assert workflow.validation.completed == [
        VisualizationMode.STREAMLINES,
        VisualizationMode.NORMAL,
    ]
    assert workflow.refreshed_cache == 2
...
class _Controller:
    def __init__(self, *, available: bool = True, state: str = "VALID") -> None:
