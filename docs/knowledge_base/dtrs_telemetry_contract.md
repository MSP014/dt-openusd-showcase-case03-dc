# Digital Twin Runtime Suite Telemetry Provider Contract

This document defines the telemetry vocabulary for Digital Twin Runtime Suite
(DTRS).

The contract has two layers:

- the **Stage 3 synthetic subset**, which is the small node-level data set that
  the current DTRS roadmap should implement first;
...
DTRS should consume telemetry through a provider boundary, not directly from UI
...
- `timestamp`: DTRS receive or generation time.
...
connector are future implementation work. DTRS does not require an external
source to use canonical DTRS metric IDs. The adapter is responsible for
translating its source schema into `TelemetrySnapshot` before DTRS consumes it.
...
vendor APIs. Those concerns belong at the adapter boundary, not in DTRS runtime
...
| `workload_percent` | node/rack | DTRS workload driver or live utilisation proxy. |
...
| `last_update_age_ms` | all | Helps DTRS detect stale data. |
...
| Metric | Scope | Notes |
| :--- | :--- | :--- |
| `cpu_fan_rpm` | component/node | First fan-motion target for DTRS. |
...
| `node_airflow_cfm` | node | Estimated physical airflow exposed by DTRS. |
...
consumer/workstation PSU. DTRS must not imply that this PSU exposes server-class
...
- DTRS consumers must tolerate missing optional fields.
- Provider adapters must normalise units before values enter the DTRS state
  layer.
